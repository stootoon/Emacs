...
(defvar matlab-math-keyword-face 'matlab-math-keyword-face
  "Self reference for math keywords.")

(defvar matlab-array-keyword-face 'matlab-array-keyword-face
  "Self reference for array keywords.")

(defvar matlab-plot-keyword-face 'matlab-plot-keyword-face
  "Self reference for array keywords.")

(defvar matlab-print-keyword-face 'matlab-print-keyword-face
  "Self reference for array keywords.")
...
If font lock is not loaded, lay in wait."
  (if (and (featurep 'custom) (fboundp 'custom-declare-variable))
	...
	(defface matlab-math-keyword-face
	  (list
	   (list t
		 (list :foreground "dodgerblue"
		       :bold t)))
	  "*Face to use for math keywords.")

	(defface matlab-plot-keyword-face
	  (list
	   (list t
		 (list :foreground "violet"
		       :bold t)))
	  "*Face to use for plotting keywords.")

	(defface matlab-array-keyword-face
	  (list
	   (list t
		 (list :foreground "skyblue1"
		       :bold t)))
	  "*Face to use for plotting keywords.")

	(defface matlab-print-keyword-face
	  (list
	   (list t
		 (list :foreground "lavender"
		       :bold t)))
	  "*Face to use for printing keywords.")
...
(defcustom matlab-keyword-list '("global" "persistent" "for" "parfor" "while"
				 "spmd" "if" "elseif" "else" "on" "off"
				 "endfunction" "return" "break" "continue"
				 "switch" "case" "otherwise" "try"
				 "catch" "tic" "toc" "struct" "load" "save" "true" "false"
				 ;; MCOS keywords
...
(defcustom matlab-print-keyword-list '("sprintf" "fprintf" "disp"
				 )
  "List of plot keywords for MATLAB used in highlighting.
Customizing this variable is only useful if `regexp-opt' is available."
  :group 'matlab
  :type '(repeat (string :tag "Keyword: ")))

(defcustom matlab-plot-keyword-list '("plot" "imagesc" "image" "clf" "cla" "xlim" "ylim" "xlabel" "ylabel" "title" "subplot" "legend" "hold" "box" "legend"
				 )
  "List of plot keywords for MATLAB used in highlighting.
Customizing this variable is only useful if `regexp-opt' is available."
  :group 'matlab
...
(defcustom matlab-array-keyword-list '("cellfun" "arrayfun" "find" "any" "all" "numel" "size" "sort" "unique"
				 )
  "List of array keywords for MATLAB used in highlighting.
Customizing this variable is only useful if `regexp-opt' is available."
  :group 'matlab
  :type '(repeat (string :tag "Keyword: ")))


(defcustom matlab-math-keyword-list '("sin" "asin" "cos" "acos" "tan" "atan"
				 "rand" "randn" "mean" "sum" "std" "var" "corr" "corrcoef" "median" "prctile"
				 "cov" "zeros" "ones" "sqrt"
				 "log" "log10" "linspace" "logspace" "exp" "pow" "minus"
				 "min" "max" "argmin" "argmax" "isequal"
				 "double"
				 )
  "List of math keywords for MATLAB used in highlighting.
Customizing this variable is only useful if `regexp-opt' is available."
  :group 'matlab
  :type '(repeat (string :tag "Keyword: ")))


(defcustom matlab-handle-graphics-list '("figure" "axes" "axis" "line" "sfigure"
...
\\|\\(par\\)?for\\|spmd\\|global\\|if\\|otherwise\\|return\\|switch\\|try\\|while\\|tic\\|toc\\)\\>")
   ...
   ;; Math keywords
   (list
    (concat "\\<\\(" (regexp-opt matlab-math-keyword-list) "\\)\\>")
    '(0 'matlab-math-keyword-face))
   ;; Plot keywords
   (list
    (concat "\\<\\(" (regexp-opt matlab-plot-keyword-list) "\\)\\>")
    '(0 'matlab-plot-keyword-face))
   ;; Print keywords
   (list
    (concat "\\<\\(" (regexp-opt matlab-print-keyword-list) "\\)\\>")
    '(0 'matlab-print-keyword-face))
   ;; Array keywords
   (list
    (concat "\\<\\(" (regexp-opt matlab-array-keyword-list) "\\)\\>")
    '(0 'matlab-array-keyword-face))
  ...
  (if (string-match matlab-anchor-end str)
      (save-excursion
        (while (re-search-backward matlab-anchor-beg
				   ...
				   (max (point-min) (- (point-max) 819)) ; Used to be 8192, reducing to reduce slowdown.
				   t)
